...
function AddMed() {
    ...
    const handlerSubmitMED = async (event) => {
        ...
        try {
            // Estimate gas
            const gasEstimate = await SupplyChain.methods.addMedicine(MedName, MedDes).estimateGas({ from: currentaccount });

            // Send transaction with estimated gas limit
            const receipt = await SupplyChain.methods.addMedicine(MedName, MedDes).send({ from: currentaccount, gas: gasEstimate });
